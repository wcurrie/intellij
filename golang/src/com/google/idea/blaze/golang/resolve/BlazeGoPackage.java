...
public class BlazeGoPackage extends GoPackageCompatAdapter {
  ...
  public static BlazeGoPackage create(
    ...
    TargetKey targetKey = getRepresentativeTarget(targets);
    targetKey = replaceProtoLibrary(project, projectData, targetKey);
  ...
  private static TargetKey getRepresentativeTarget(Collection<TargetIdeInfo> targets) {
    for (TargetIdeInfo target : targets) {
      if (target.getKind().ruleType != RuleType.TEST) {
        return target.getKey();
      }
    }
    return targets.iterator().next().getKey();
  }
  ...
  public Collection files() {
    if (cachedGoFiles == null) {
      ...
      for (GoFile file : cachedGoFiles) {
        ...
        if (filePackageName != null && !filePackageName.equals(getName())) {
